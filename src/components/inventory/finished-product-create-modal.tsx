...
import { ExternalLink, PackagePlus, Plus, RefreshCw, X } from "lucide-react";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4">
          <div className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-3xl border bg-card shadow-2xl">
            <div className="flex items-center justify-between gap-3 border-b px-5 py-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Catalogo conectado</p>
            ...
            <div className="grid max-h-[calc(92vh-73px)] gap-5 overflow-y-auto p-5">
              <section className="grid gap-4 rounded-2xl border bg-card-muted/45 p-4 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  <p className="text-sm font-semibold">Crear desde catalogo</p>
                  <p className="mt-1 text-sm text-muted">Entra con tu sesion de inventario, publica el producto y sincroniza.</p>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row md:justify-end">
                  <a className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-accent px-4 text-sm font-semibold text-accent-foreground transition hover:opacity-90" href="/api/catalog/handoff" target="_blank">
                    <ExternalLink size={16} /> Entrar al catalogo
                  </a>
                  <form action={syncCatalogProductsAction}>
                    <button className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-full border bg-card px-4 text-sm font-semibold transition hover:bg-card-muted" type="submit">
              ...
              <button className="inline-flex h-10 w-fit items-center justify-center gap-2 rounded-full border bg-card px-4 text-sm font-semibold transition hover:bg-card-muted" onClick={() => setShowLocalForm((value) => !value)} type="button">
                <PackagePlus size={16} /> {showLocalForm ? "Ocultar creacion local" : "Crear localmente de emergencia"}
              </button>

              {showLocalForm ? (
                <form action={createFinishedProductAction} className="grid gap-4 rounded-2xl border p-4">
                  ...
                  <div className="flex justify-end">
                    <Button disabled={warehouses.length === 0} type="submit"><PackagePlus size={16} /> Guardar producto local</Button>
                  </div>
                </form>
              ) : null}
            </div>
